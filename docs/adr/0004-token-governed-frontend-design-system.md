...
`docs/frontend-design-system.md` also defines the operating boundary between primitives, reusable UI components, and direct Emotion styled components. Primitives are the preferred tool for simple token-aware layout, spacing, grid/flex composition, and typography. Reusable UI components own shared semantic controls and product surfaces. Direct Emotion remains valid in Song Library feature code when a component needs complex states, pseudo-selectors, responsive recipes, table-specific behavior, custom controls, animation, or dynamic visual surfaces, as long as those styles continue to use theme tokens and Song Library terminology.
